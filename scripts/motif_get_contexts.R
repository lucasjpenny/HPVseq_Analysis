...
calculate_mds <- function(frequencies) {
  # Normalize the frequencies to sum to 1 if they don't already
  frequencies <- frequencies / sum(frequencies)
  # Calculate the MDS using Shannon entropy
  mds <- -sum(frequencies * log(frequencies)) / log(length(frequencies))
  return(mds)
}

calculate_mds <- calculate_mds(motif[, c("frequency")])
...
write.table(calculate_mds, file.path(outdir, paste0(id, "_MDS.txt")), sep = "\t", row.names = FALSE)
